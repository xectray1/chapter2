...
Library:Notify("waiting for game to load", 5);
if not (game:IsLoaded()) then game.Loaded:Wait(); end;
local DadFolder = workspace:WaitForChild("Game"):WaitForChild("dad")
local DadModel = DadFolder:FindFirstChild("PossesedDad")
while not DadModel do
    DadModel = DadFolder:WaitForChild("PossesedDad") or DadFolder:WaitForChild("Dad")
end
local Window = Library:CreateWindow({
    Title = "                         skripthookv   weird strict dad(chapter 2)",
    Center = true,
    AutoShow = true,
})
...
local lockers = Workspace:WaitForChild("Basement"):WaitForChild("Lockers")
local chaseDoorBool = Workspace:WaitForChild("Game"):WaitForChild("dad"):WaitForChild("PossesedDad"):WaitForChild("ChaseDoor")
local connection
function AutoHideEnabled(enabled)
    if enabled then
        connection = chaseDoorBool.Changed:Connect(function(newValue)
            if newValue == true then
                local character = player.Character or player.CharacterAdded:Wait()
                local rootPart = character:FindFirstChild("HumanoidRootPart")
                if not rootPart then return end

                local locker = lockers:GetChildren()[5]
                if not locker then return end

                local primary = locker:FindFirstChild("Primary")
                local prompt = primary and primary:FindFirstChildOfClass("ProximityPrompt")
                if not (primary and prompt and prompt.Enabled) then return end

                character:PivotTo(primary.CFrame)
...
MainBox:AddToggle("AutoHide", {
    Text = "auto hide",
    Default = false,
    Callback = AutoHideEnabled,
})
local DadModel = Workspace:WaitForChild("Game"):WaitForChild("dad"):WaitForChild("PossesedDad")
...
text.Outline = true
local connection
local function ToggleDadESP(enabled)
    if enabled then
        connection = RunService.RenderStepped:Connect(function()
            if not DadModel or not DadModel.PrimaryPart then
                box.Visible = false
                text.Visible = false
                return
            end
            local rootPos, onScreen = camera:WorldToViewportPoint(DadModel.PrimaryPart.Position)
            ...
            local corners = {
                DadModel.PrimaryPart.CFrame * CFrame.new( size.X,  size.Y,  size.Z),
                DadModel.PrimaryPart.CFrame * CFrame.new( size.X,  size.Y, -size.Z),
                DadModel.PrimaryPart.CFrame * CFrame.new( size.X, -size.Y,  size.Z),
                DadModel.PrimaryPart.CFrame * CFrame.new( size.X, -size.Y, -size.Z),
                DadModel.PrimaryPart.CFrame * CFrame.new(-size.X,  size.Y,  size.Z),
...
MainBox:AddToggle("ESPDad", {
    Text = "esp dad",
    Default = false,
    Callback = ToggleDadESP,
})
